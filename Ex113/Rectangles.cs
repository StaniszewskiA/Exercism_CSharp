...
public static class Rectangles
{
    ...
    private static CellType[][] ParseGrid(string[] rows) => rows.Select(row => row.Select(ParseCell).ToArray()).ToArray();

    private static CellType ParseCell(char cell)
    {
        switch (cell)
        {
            case '+': return CellType.Corner;
            case '-': return CellType.HorizontalLine;
            case '|': return CellType.VerticalLine;
    ...
    private static bool ConnectsVertically(Point point, CellType[][] grid) => (Cell(point, grid) == CellType.VerticalLine) || (Cell(point, grid) == CellType.Corner);
